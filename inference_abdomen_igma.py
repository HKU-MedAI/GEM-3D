...
resume_path = './infer_model/abdomen'
...
slice_logdir = resume_path.rstrip("/")
ckpt = os.path.join(slice_logdir, "checkpoints", "last.ckpt")

base_configs = sorted(glob.glob(os.path.join(slice_logdir, "configs/*.yaml")))[1]
configs = OmegaConf.load(base_configs)
slice_model = instantiate_from_config(configs.model)
slice_model.init_from_ckpt(ckpt)
slice_model.eval()
...
config = OmegaConf.load('./configs/latent-diffusion/inference_abdomen.yaml')
...
for idx, data in tqdm(enumerate(valloader)):
    ...
    aug_seg_tmp = torch.ones_like(aug_seg) * (-1)
    aug_seg_tmp = aug_seg_tmp + 0.1
    aug_seg = aug_seg + 0.1
    aug_seg = torch.from_numpy(aug(**{'image':aug_seg.squeeze().numpy()})['image']).unsqueeze(0).unsqueeze(-1)
    mask = copy.deepcopy(aug_seg)
    aug_seg_tmp[(mask!=0).bool()] = aug_seg[(mask!=0).bool()]
    aug_seg = aug_seg_tmp - 0.1
    ...
    pos_id = torch.Tensor([start_slice / slice_num]).to(model.device)
    slice_c = slice_model.get_learned_conditioning(pos_id)
    ref_slice, _ = slice_model.sample_log(cond=slice_c, batch_size=1, ddim=True, eta=1., ddim_steps=200)
    z_ref_0 = ref_slice.detach()
    z_ref = z_ref_0.clone()
    ...
    x_result = np.rot90(x_result, k=3, axes=(0,1))
